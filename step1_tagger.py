...
FEEDBACK_SUFFIX = """
---
上一次标注结果已经给用户看过了，用户有以下修改意见：

"{feedback}"

请根据这个意见重新调整话题分组，其他合理的部分保持不变。
"""

def run(srt_path, out_dir, model_name=None, feedback=None):
    ...
    if feedback:
        prompt += FEEDBACK_SUFFIX.replace("{feedback}", feedback)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feedback', default=None, help='用户自然语言修改意见，触发重新标注')
    args = parser.parse_args()
    run(args.srt, args.out, args.model, args.feedback)
